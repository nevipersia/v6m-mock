...
const STAGES = ['inquiry', 'ocular', 'reserved', 'paid', 'done'];

function eventCard(ctx, event) {
  const { state } = ctx;
  const pkg = findPackage(state, event.packageId);
  const booking = event.bookingId && findBooking(state, event.bookingId);
  ...
  return html`
    <article class="event-card">
      ...
      <p class="small muted">${pkg.name} · ${plural(event.guests, 'guest')}${event.exclusive ? ' · closes resort' : ''}</p>
