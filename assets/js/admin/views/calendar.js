...
const ui = { range: 'week', anchor: null };

const anchorOf = (state) => ui.anchor ?? state.meta.asOf;

const startOfWeek = (date) => addDays(date, -((parseDate(date).getDay() + 6) % 7)); // Monday
const startOfMonth = (date) => `${date.slice(0, 8)}01`;

function daysInView(state) {
...
function rangeLabel(state) {
  ...
  if (ui.range === 'day') return formatDate(days[0], 'long');
  if (ui.range === 'month') return parseDate(days[0]).toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
  return `${formatDate(days[0], 'monthDayLong')} – ${formatDate(days[days.length - 1], 'monthDayLong')}`;
}
...
const bookingsOn = (state, day) => state.bookings.filter((booking) => isActive(booking) && booking.date === day);

// ---------- Day view ----------

function dayView(ctx, day) {
  const { state } = ctx;
...
function monthView(ctx, days) {
  const { state } = ctx;
  const leading = (parseDate(days[0]).getDay() + 6) % 7; // Monday-first grid
  ...
  return html`
    ...
    <div class="calendar-bar">
      ...
      <label class="field calendar-bar__jump">
        ...
        <input class="input" type="date" data-input="jump" value="${days[0]}">
      </label>
    </div>

    ${ui.range === 'day' ? dayView(ctx, days[0]) : ui.range === 'week' ? weekView(ctx, days) : monthView(ctx, days)}
