...
function thread(ctx, inquiry) {
  const { state } = ctx;
  const guest = findGuest(state, inquiry.guestId);
  const booking = inquiry.relatedBookingId && findBooking(state, inquiry.relatedBookingId);
  const assignee = inquiry.assignedTo && findStaff(state, inquiry.assignedTo);
