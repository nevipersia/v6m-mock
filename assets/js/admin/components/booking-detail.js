...
const methodOptions = (selected) => ['gcash', 'cash', 'bank_transfer'].map((method) =>
  html`<option value="${method}" ${method === selected ? 'selected' : ''}>${METHOD_LABELS[method]}</option>`);

function guestsText(booking) {
  const parts = [plural(booking.adults, 'adult')];
  if (booking.kids) parts.push(plural(booking.kids, 'kid'));
  return parts.join(', ');
}

function facts(state, booking) {
  const guest = findGuest(state, booking.guestId);
  const event = booking.eventId && state.events.find((item) => item.id === booking.eventId);
  const pkg = event && findPackage(state, event.packageId);
  ...
  if (pkg) rows.push(['Package', html`${pkg.name} ${stagePill(event.stage)}`]);
...
function paymentsSection(state, booking) {
...
function activitySection(state, booking) {
...
export function createBookingDetail(bookingId) {
  const ui = { checkInOpen: false, confirmCancel: false, errors: {} };

  function actionsSection(ctx, booking) {
    const { state } = ctx;
    const parts = [];
  ...
  return {
    ...
    actions: {
      ...
      'confirm-check-in': ({ root, ctx, redraw }) => {
        if (!$('[name="idChecked"]', root).checked) {
          ui.errors.checkIn = 'Check a valid ID first.';
          redraw();
          return;
        }
        const method = $('[name="checkInMethod"]', root)?.value ?? 'cash';
        const booking = findBooking(ctx.state, bookingId);
      ...
      'record-payment': ({ el, ctx, redraw }) => {
        const booking = findBooking(ctx.state, bookingId);
        const amount = Math.round(Number(el.elements.amount.value));
        if (!(amount > 0)) {
        ...
        recordPayment(bookingId, {
          amount,
          method: el.elements.method.value,
          reference: el.elements.reference.value.trim(),
        }, ctx.staff.id);
      ...
      'confirm-cancel': ({ root, ctx }) => {
        const reason = $('[name="cancelReason"]', root).value;
        ui.confirmCancel = false;
