...
import { createPdf, downloadBlob, pdfPage } from '../../core/pdf.js';
...
const NAVY = [0.12, 0.18, 0.36];

export function bookingPdfBlob(state, booking) {
  const guest = findGuest(state, booking.guestId);
  const staff = findStaff(state, booking.createdBy);
  const items = [];
  let y = pdfPage.height - MARGIN;

  const text = (value, { x = MARGIN, size = 11, bold = false, gray = 0.15 } = {}) => {
...
export function downloadBookingPdf(state, booking) {
