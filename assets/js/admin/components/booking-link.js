...
const linkUrl = (code) => new URL(`../book/?code=${encodeURIComponent(code)}`, window.location.href).href;

function productOptions(state, selected) {
  return [
...
function sentLinks(state) {
  return state.bookingLinks
    .filter((link) => link.status === 'sent')
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function createBookingLinkPanel() {
  const draft = { product: '', date: '', note: '', expiresInDays: 7 };
  const ui = { created: null, error: '' };

  function createdBlock(state) {
  ...
  return {
    ...
    inputs: {
      field: ({ el }) => {
        draft[el.name] = el.name === 'expiresInDays' ? Number(el.value) : el.value;
        ui.error = '';
      },
    },
